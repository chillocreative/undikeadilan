        ...
        <form method="POST" action="{{ route('admin.candidates.sync', $election->slug) }}" x-data="{ selected: [] }">
            ...
            <div class="space-y-2 mb-4 max-h-96 overflow-y-auto">
                ...
                <div class="flex items-center gap-2 rounded-xl bg-white/5 px-4 py-3 ring-1 ring-white/10">
                    ...
                    <form method="POST" action="{{ route('admin.nominee.delete', $election->slug) }}" onsubmit="return confirm('Padam pencalonan {{ $nominee->name }}?')">
                        @csrf
                        @method('DELETE')
                        <input type="hidden" name="name" value="{{ $nominee->name }}">
                        <button type="submit" class="text-rose-400/60 hover:text-rose-300 transition-colors p-1" title="Padam">
